  ...
  try {
    ...
    feedbackKeys.forEach((key) => {
      ...
      if (typeof value === "string" && value.trim() !== "") {
        ...
        textualFeedbackParts.push(
          `Ответ на вопрос "${questionLabel}":\n${value}`
        );
      }
    });
    ...
    if (npsScore !== undefined) {
      const npsQuestion = surveyQuestions.find(
        (q) => q.name === "quality_rating"
      );
      textualFeedbackParts.push(
        `Ответ на вопрос "${
          npsQuestion?.label || "Оценка качества обучения"
        }": ${npsScore}/10`
      );
    }
    ...
    if (textualFeedbackParts.length === 0 && responseId) {
      return {
        summary:
          "Ваш ответ успешно сохранен. Текстовых отзывов для анализа не предоставлено.",
        responseId,
      };
    }
    ...
    if (textualFeedbackParts.length === 0 && !responseId) {
      return {
        summary:
          "Не предоставлено текстовых отзывов для анализа. Ответ не был сохранен.",
        responseId,
      };
    }
